...
void Player::Draw()
{
	D3D.WorkShaderManager().m_HD2DShader.DrawSquarePolygon(*m_pPoly, m_mWorld);
}

void Player::Update()
{
	if (!m_pCamera->GetSpot() && m_bCanMove)//スポット中は動けない
	{
		Move();
	}
...
void Player::Init()
{
	...
	m_pPoly = std::make_shared<KdSquarePolygon>();
	m_pPoly->SetMaterial("Asset/Textures/Character/Player/Player.png");
	m_pPoly->SetScale({ 0.5f, 0.5f });
	m_pPoly->SetPivot(KdSquarePolygon::PivotType::Center_Bottom);
...
void Player::UpdateCollision()
{
	...
	if (Terrain)
	{
		...
		if (moveVal_y > 0)
		{
			...
			//天井判定------------------------------------------------------------------------------------------------
			if (Terrain->GetCollider()->Intersects(KdCollider::TypeCeiling, rayInfo, &result))//返り値bool
			{
				//当たり判定をもとに、地面の上に立つ処理
				...
				if (pNearestRes)
				{
					...
					Math::Vector3 pos = pNearestRes->m_hitPos;
					pos.y -= 0.5;
