...
Application::~Application() {}
const std::unique_ptr<KdGameSystem>& const Application::GetObj() const { return GameSystem; }
std::unique_ptr<KdGameSystem>& Application::WorkObj() { return GameSystem; }
...
bool Application::Init(int w, int h)
{
	...
	// ウィンドウ作成
	//===================================================================
	if (m_window.Create(w, h, "Box World", "Window") == false) {
...
void Application::Execute()
{
	...
	// 初期化
	GameSystem = std::make_unique<KdGameSystem>();
	GameSystem->Init();
	...
	while (1)
	{
		...
		// ゲーム描画
		Math::Color color(0.5f, 1.0f, 0.0f, 1.0f);
		//描画のすべてを担っている      backbafferの初期化
		D3D.WorkDevContext()->ClearRenderTargetView(D3D.WorkBackBuffer()->WorkRTView(), color);
