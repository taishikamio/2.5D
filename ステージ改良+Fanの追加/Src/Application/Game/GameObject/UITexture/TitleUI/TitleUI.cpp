...
void TitleUI::Init()
{
	...
	if (m_pStartTex)m_pStartTex->Load("Asset/Textures/UI/PressEnter.png");
}
...
void TitleUI::Draw2D()
{
	if (m_pTitleLogo)
	{
		...
		D3D.WorkShaderManager().m_spriteShader.DrawTex(m_pTitleLogo.get(), 400, 200);

		D3D.WorkShaderManager().m_spriteShader.DrawTex(m_pStartTex.get(), 400, 0, nullptr, &Math::Color(1, 1, 1, alpha));
