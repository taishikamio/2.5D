...
void KdGameSystem::InitGame()
{
	...
	//ギミック壁
	if (!m_pMoveWall.size())m_pMoveWall.push_back(std::make_shared<MoveWall>(Math::Vector3(-4, 5, 4), 90));
	if (m_pMoveWall.size() == 1)m_pMoveWall.push_back(std::make_shared<MoveWall>(Math::Vector3(-2, 2, 3), 0));
	...
	//ファン
	m_pFan = std::make_shared<Fan>();
	if (m_pFan)
	{
		m_pFan->SetAsset("Asset/Models/Stage/Fan/Fan.gltf");
		m_pFan->Init(Math::Vector3{ -2.5,-0.3,21.5 });
	}
	//風の判定
	m_pWind = std::make_shared<HitBoxWind>();
	if (m_pWind)
	{
		m_pWind->SetAsset("Asset/Models/Stage/Wind/HitBoxWind.gltf");
		m_pWind->Init(m_pFan->GetPos());
	}
...
void KdGameSystem::UpdateGame()
{
	...
	if (m_pFan)m_pFan->Update();
	if (m_pWind)m_pWind->Update();
...
void KdGameSystem::DrawGame()
{
	...
	D3D.WorkShaderManager().m_HD2DShader.BeginLighting();
	{
		...
		if (m_pFan)m_pFan->Draw();
		if (m_pWind)m_pWind->Draw();
